...
&lt;!DOCTYPE fo:root [
          ...
          <fo:block font-size="10pt">
            <fo:table table-layout="fixed" inline-progression-dimension="100%">
              ...
              <fo:table-body>
                <fo:table-row>
                  <fo:table-cell>
                    <fo:block text-align="start">Seite 
                      <fo:page-number/> 
                      von <fo:page-number-citation-last 
                      ref-id="DocumentBody"/>
                    </fo:block>
                  </fo:table-cell>
                  <fo:table-cell>
  ...
  <!-- ============================================
    ...
    <fo:flow flow-name="xsl-region-body">
     ...
     <fo:block break-before="page">
     	<xsl:apply-templates select="//*[@id='coverpage']"/>
     </fo:block>
  ...
  <xsl:template match="h1">
    ...
    <fo:block font-size="28pt" line-height="32pt"
      keep-with-next="always"
      space-after="22pt" font-family="sans-serif">
  ...
  <!-- ============================================
    ...
    the value "300px". Max width: 450px, Max Height: 768px
    =============================================== -->

  <xsl:template match="img">
  <xsl:variable name="imgStyle">
  	  <xsl:value-of select="concat(concat('*',translate(@style, ' ','')),'*')"/>
    </xsl:variable>
   <xsl:variable name="imgWidth">
   	<xsl:choose>
     		<xsl:when test="contains($imgStyle, '*width') or contains($imgStyle, ';width')">
     			<xsl:if test="contains($imgStyle, '*width')">
	          	<xsl:value-of select="substring-before(substring-after($imgStyle, '*width:'), ';')"/>
	       </xsl:if>
	       <xsl:if test="contains($imgStyle, ';width')">
          	<xsl:value-of select="substring-before(substring-after($imgStyle, ';width:'), ';')"/>
	       </xsl:if>
     		...
     		<xsl:otherwise>
	            ...
	            <xsl:choose>
	              <xsl:when test="contains(@width, 'px')">
	                <xsl:value-of select="number(@width) * 0.7"/>
	              </xsl:when>
	              <xsl:otherwise>
	                <xsl:value-of select="concat(number(@width) * 0.7, 'px')"/>
	              </xsl:otherwise>
	            </xsl:choose>
	        </xsl:if>
     		</xsl:otherwise>
    ...
    <fo:block>
      <fo:external-graphic src="{@src}">
      	<xsl:attribute name="content-width">
      		<xsl:choose>
      			<xsl:when test="number(translate($imgWidth, 'px', '')) > 450">
      				<xsl:value-of select="'450px'" />
      			</xsl:when>
      			<xsl:otherwise>
  ...
  <fo:block font-size="12pt" line-height="15pt" space-after="12pt">
  	<xsl:call-template name="build-formatted-text">
  ...
  <xsl:template match="span">
    ...
    <xsl:variable name="spanfontsize">
      <xsl:choose>
	    <xsl:when test="contains($spanstyle, 'font-size:8px;')">
	        <xsl:value-of select="'7pt'"/>
	    </xsl:when>
	    <xsl:when test="contains($spanstyle, 'font-size:9px;')">
	        <xsl:value-of select="'8pt'"/>
	    </xsl:when>
	    <xsl:when test="contains($spanstyle, 'font-size:10px;')">
	        <xsl:value-of select="'9pt'"/>
	    </xsl:when>
	    <xsl:when test="contains($spanstyle, 'font-size:11px;')">
	        <xsl:value-of select="'10pt'"/>
	    </xsl:when>
	    <xsl:when test="contains($spanstyle, 'font-size:12px;')">
	        <xsl:value-of select="'11pt'"/>
	    </xsl:when>
	    <xsl:when test="contains($spanstyle, 'font-size:16px;')">
	        <xsl:value-of select="'14pt'"/>
	    </xsl:when>
	    <xsl:when test="contains($spanstyle, 'font-size:18px;')">
    ...
    <fo:inline>
        <xsl:attribute name="font-size">
          <xsl:value-of select="$spanfontsize"/>
        </xsl:attribute>
        <xsl:call-template name="build-formatted-text">
           <xsl:with-param name="textStyle" select="$spanstyle"/>
        </xsl:call-template>
	    <xsl:apply-templates select="*|text()"/>
	</fo:inline>
  ...
  <xsl:template match="table">
      ...
      <xsl:for-each select="tbody/tr[1]/td">
       	...
       	<xsl:variable name="colWidth">
       		<xsl:value-of select="concat(substring-before(substring-after($columnStyle, '*width:'), ';'), substring-before(substring-after($columnStyle, ';width:'), ';'))"/>
       	</xsl:variable>
       	<xsl:choose>
       		<xsl:when test="@colspan">
       			<xsl:call-template name="build-colspan">
		           <xsl:with-param name="cols-remaining" select="number(@colspan)"/>
		           <xsl:with-param name="colWidth" select="concat(round(number(translate($colWidth, '%', ''))) div number(@colspan)	,'%')"/>
          		</xsl:call-template>
       		</xsl:when>
       		<xsl:otherwise>
       			<fo:table-column>
       				<xsl:attribute name="column-width">
       					<xsl:value-of select="concat(round(number(translate($colWidth, '%', ''))), '%')"/>
       				</xsl:attribute>
       			</fo:table-column>
       		</xsl:otherwise>
       	</xsl:choose>
  		<xsl:if test="@colspan">
	  		<xsl:text>has colspan:</xsl:text>
  ...
  <xsl:template match="td">
    <fo:table-cell 
      ...
      <xsl:if test="contains($cellStyle, 'border-color')">
      	<xsl:attribute name="border-color">
      		<xsl:value-of select="substring-before(substring-after($cellStyle, 'border-color:'), ';')"></xsl:value-of>
      	</xsl:attribute>
      	<xsl:attribute name="border-width">
          <xsl:text>1pt</xsl:text>
        </xsl:attribute>
      ...
      <xsl:if test="contains($cellStyle, 'background-color')">
      	<xsl:attribute name="background-color">
      		<xsl:value-of select="substring-before(substring-after($cellStyle, 'background-color:'), ';')"/>
      	</xsl:attribute>
      </xsl:if>
      
      <xsl:if test="contains($cellStyle, 'border-width')">
      	<xsl:attribute name="border-width">
      		<xsl:value-of select="substring-before(substring-after($cellStyle, 'border-width:'), ';')"/>
      	</xsl:attribute>
      </xsl:if>
      <xsl:if test="contains($cellStyle, 'border-style')">
      	<xsl:attribute name="border-style">
      		<xsl:value-of select="substring-before(substring-after($cellStyle, 'border-style:'), ';')"/>
      	</xsl:attribute>
      </xsl:if>
      
      <!-- Vertical Alignment -->
      
      <xsl:if test="contains($cellStyle, 'vertical-align:')">
      	<xsl:attribute name="display-align">
      		<xsl:value-of select="substring-before(substring-after($cellStyle, 'vertical-align:'), ';')"/>
      	</xsl:attribute>
      </xsl:if>

      <fo:block>
	      <xsl:call-template name="build-formatted-text">
	      	<xsl:with-param name="textStyle" select="$cellStyle"/>
	      </xsl:call-template>
        <xsl:apply-templates select="*|text()"/>
      </fo:block>
    </fo:table-cell>
  ...
  <xsl:template name="toc">
    ...
    <fo:block line-height="11pt" font-size="8pt" space-after="10pt">
    </fo:block>
  ...
  <xsl:template name="build-colspan">
  	<xsl:param name="cols-remaining"/>
  	<xsl:param name="colWidth"/>
  	
  	<xsl:if test="$cols-remaining > 0">
  		<fo:table-column>
    		<xsl:attribute name="column-width">
  		...
  		<xsl:call-template name="build-colspan">
        	<xsl:with-param name="cols-remaining" select="$cols-remaining - 1"/>
        	<xsl:with-param name="colWidth" select="$colWidth"/>
      </xsl:call-template>
  	</xsl:if>
  </xsl:template>
  <xsl:template name="build-formatted-text">
  	<xsl:param name="textStyle"/>
  	<xsl:variable name="textColor">
  		<xsl:value-of select="concat(substring-before(substring-after($textStyle, '*color:'), ';'),substring-before(substring-after($textStyle, ';color:'), ';'))"/>
  	</xsl:variable>
  	<xsl:variable name="textBack">
  		<xsl:value-of select="substring-before(substring-after($textStyle, 'background-color:'), ';')"/>
  	</xsl:variable>
  	<xsl:variable name="textAlign">
  		<xsl:value-of select="substring-before(substring-after($textStyle, 'text-align:'), ';')"/>
  	</xsl:variable>
	...
	<xsl:if test="$textBack">
		<xsl:attribute name="background-color">
			<xsl:value-of select="$textBack"/>
		</xsl:attribute>
	</xsl:if> 
